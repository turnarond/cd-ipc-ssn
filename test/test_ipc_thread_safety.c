...
static void on_command_test(struct ipc_client *client, ipc_header_t *ipc_hdr, ipc_data_ref_t *data, void *arg)
{
    if (ipc_hdr) {
...
static void *thread_func(void *arg)
{
    ...
    // 测试连接
    if (!ipc_client_connect(client, "ipc-test_server", NULL)) {
        pthread_mutex_lock(&print_mutex);
    ...
    for (i = 0; i < TEST_ITERATIONS; i++) {
        ...
        int ret = ipc_client_call(client, &url, &data, on_command_test, (void *)thread_id, 1000);
        if (ret < 0) {
            pthread_mutex_lock(&print_mutex);
        ...
        // 测试轮询
        ipc_client_poll(client, 10);

        // 测试设置发送超时
        ipc_client_send_timeout(client, 1000);

        // 测试设置消息处理回调
        ipc_client_set_on_message(client, NULL, NULL);
