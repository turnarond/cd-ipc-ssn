...
#define TEST_PEER_ADDRESS "127.0.0.1:8888"
 ...
 */
static void test_connect_handler(ipc_server_t *server, cli_id_t id, bool connect, void *arg)
{
    (void)server;
    (void)id;
    (void)arg;
 ...
 */
static void test_rpc_reply_handler(ipc_client_t *client, ipc_header_t *ipc_hdr, 
...
static bool test_node_pubsub(void)
{
    ...
    if (!ipc_node_subscribe(client_node, &topic, test_message_handler, NULL, TEST_TIMEOUT)) {
    ...
    // Publish message
    ipc_data_ref_t message = {
        .data = "test message",
        .length = 12
    };
    
    if (!ipc_node_publish(server_node, &topic, &message)) {
...
static bool test_node_rpc(void)
{
    ...
    // Add RPC method
    ipc_url_ref_t method = {
        .url = "/test/rpc",
        .url_len = 9
    };
    
    if (!ipc_node_add_rpc_method(server_node, &method, test_rpc_handler, NULL)) {
    ...
    ipc_data_ref_t request = {
        .data = "RPC request",
        .length = 12
    };
    
    int result = ipc_node_rpc_call(client_node, TEST_PEER_ADDRESS, &method, &request,
