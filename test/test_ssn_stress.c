...
static void on_command_stress(ssn_client_t *client, ssn_header_t *ssn_hdr, ssn_data_ref_t *data, void *arg)
{
    // 简单的回调处理
}
...
static void *thread_func(void *arg)
{
    ...
    // 测试连接
    if (!ssn_client_connect(client, "unix:///tmp/ipc-stress_server", NULL)) {
    ...
    for (i = 0; i < TEST_ITERATIONS; i++) {
        ...
        int ret = ssn_client_call(client, &url, &data, on_command_stress, NULL, 500);
        if (ret < 0) {
            thread_failure++;
        } else {
            thread_success++;
