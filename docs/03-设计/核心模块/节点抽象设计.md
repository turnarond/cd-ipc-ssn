...
typedef struct {
    // 创建数据流
    ipc_stream_t* (*create_stream)(ssn_node_t* node, const char* stream_id,
...
typedef struct {
    // 获取节点统计信息
    bool (*get_statistics)(ssn_node_t* node, ipc_node_statistics_t* stats);
...
typedef struct {
    // 获取最后错误码
    ipc_node_error_t (*get_last_error)(ssn_node_t* node);
    ...
    // 错误日志记录
    void (*log_error)(ssn_node_t* node, ipc_node_error_t error,
                     const char* format, ...);
...
int main() {
    ...
    // 2. 创建节点
    ssn_node_t* node = g_ipc_node_api.create(&config);
    if (!node) {
