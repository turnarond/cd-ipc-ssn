...
本文档汇总 SSN 使用中的高频问题，按类别分组，每个问题按「问题 → 原因 → 解决方案」组织，并指向相关文档或代码位置。内容与当前版本（v2.3.1）代码行为保持一致。
...
**解决方案**：启动后在主循环或独立线程中周期调用 `ssn_server_poll(server, timeout_ms)`（timeout 为毫秒），或使用阻塞式 `ssn_server_run(server)`；客户端同理使用 `ssn_client_poll`/`ssn_client_run`。示例见 [使用指南](使用指南.md)「5.1 基础通信」、[快速上手](快速上手.md)「1.3 服务器事件循环」；行为基线详见 [部署场景指南](../05-部署手册/部署场景指南.md)「4.2 事件循环驱动」。
